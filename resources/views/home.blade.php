...
<head>
    ...
    <style>
        @font-face {
            font-family: 'll';
            src: url({{ asset('littlelink/fonts/littlelink-custom.otf') }}) format("opentype")
        }
    </style>
        ...
        <div class="row">
            <div class="sign" style="margin-top: 30px; text-align: right;">
                @if (Route::has('login'))
                    ...
                    @else
                        <a href="{{ route('login') }}" class="underline spacing">Iniciar sesión</a>

                        {{-- @if (Route::has('register') and $page->register == 'true')
                            <a href="{{ route('register') }}" class="underline spacing">Registrar</a>
            ...
            <div class="column" style="margin-top: 15%">
                <!-- Your Image Here -->
                @if (file_exists(base_path('littlelink/images/avatar.png')))
                    <img alt="avatar" src="{{ asset('littlelink/images/avatar.png') }}"
                        srcset="{{ asset('littlelink/images/avatar@2x.png 2x') }}" height="128px">
                @else
                    <div class="logo-container fadein">
                        <img class="rotate" src="{{ asset('littlelink/images/just-gear.svg') }}" alt="Logo"
                    ...
                    @foreach ($array as $button)
                        ...
                        @if ($button['button'] === 'custom' and ($button['custom_css'] === '' or $button['custom_css'] === 'NULL') or theme('allow_custom_buttons') == 'false' and $button['button'] === 'custom')
                            <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                    class="button button-{{ $button['button'] }} button button-hover icon-hover"
                                    rel="noopener noreferrer nofollow" href="{{ $button['link'] }}"
                                    @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif>
                                    @if ($button['icon'] == 'llc')
                                        <img alt="button-icon" class="icon hvr-icon"
                                        src="{{ asset('\/littlelink/icons\/') }}llc.svg">@else<i
                                            style="color: {{ $button['icon'] }}"
                                            class="icon hvr-icon fa {{ $button['icon'] }}"></i>
                                    @endif {{ $button['title'] }}
                                </a></div>
                        @elseif($button['button'] === 'custom' and $button['custom_css'] != '')
                            <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                    class="button button-hover icon-hover" style="{{ $button['custom_css'] }}"
                                    rel="noopener noreferrer nofollow" href="{{ $button['link'] }}"
                                    @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif>
                                    @if ($button['icon'] == 'llc')
                                        <img alt="button-icon" class="icon hvr-icon"
                        ...
                        @elseif($button['button'] === 'buy me a coffee')
                            <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                    ...
                                    me a Coffee</a></div>
                        @elseif($button['button'] === 'custom_website' and
                            ($button['custom_css'] === '' or $button['custom_css'] === 'NULL') or
                            theme('allow_custom_buttons') == 'false' and $button['button'] === 'custom_website')
                            <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                    class="button button-custom_website button button-hover icon-hover"
                                    rel="noopener noreferrer nofollow" href="{{ $button['link'] }}"
                                    @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif><img alt="button-icon"
                                        class="icon hvr-icon"
                                        src="https://icons.duckduckgo.com/ip3/{{ strp($button['link']) }}.ico">{{ $button['title'] }}</a>
                            </div>
                        ...
                        @else
                            <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                    class="button button-{{ $button['button'] }} button button-hover icon-hover"
                                    @if ($button['link'] != '') href="{{ $button['link'] }}" target="_blank" @endif><img
                                        alt="button-icon" class="icon hvr-icon"
                                        src="{{ asset('\/littlelink/icons\/') . $linkName }}.svg">{{ ucfirst($linkName) }}</a>
                            </div>
                        @endif
                    @endforeach
                @else
                    <div style="--delay: {{ $initial++ }}s" class="button-entrance">
                        <div class="button button-github button button-hover icon-hover"><img alt="button-icon"
                                class="icon hvr-icon" src="{{ asset('littlelink/icons/github.svg') }}">Github</div>
                    </div>
                    <div style="--delay: {{ $initial++ }}s" class="button-entrance">
                        <div class="button button-twitter button button-hover icon-hover"><img alt="button-icon"
                                class="icon hvr-icon" src="{{ asset('littlelink/icons/twitter.svg') }}">Twitter</div>
                    </div>
                    <div style="--delay: {{ $initial++ }}s" class="button-entrance">
                        <div class="button button-instagram button button-hover icon-hover"><img alt="button-icon"
                                class="icon hvr-icon" src="{{ asset('littlelink/icons/instagram.svg') }}">Instagram
                        </div>
                    </div>
                @endif --}}
                <br /><br />
                ...
                {{-- <div class="fadein">
                    @if (config('advanced-config.home_footer') == 'custom')
                        <p><?php $year = date('Y');
                        echo strtr(config('advanced-config.custom_home_footer_text'), ['{year}' => $year]); ?></p>
                    @elseif(config('advanced-config.home_footer') == 'alt')
                        <p><i style="position:relative;top:1px;" class="fa-solid fa-infinity"></i> - Button
                            combinations</p>
                    ...
                    @foreach ($updatedPages as $page)
                        @if (file_exists(base_path("img/$page->littlelink_name" . '.png')))
                            <a href="{{ url('') }}/@<?= $page->littlelink_name ?>" target="_blank">
                                <img src="{{ asset("img/$page->littlelink_name" . '.png') }}"
                                    srcset="{{ asset("img/$page->littlelink_name" . '@2x.png 2x') }}" width="50px"
                                    height="50px">
                            </a>
                        @else
                            <a href="{{ url('') }}/@<?= $page->littlelink_name ?>" target="_blank">
                                <img src="{{ asset('littlelink/images/logo.svg') }}"
                                    srcset="{{ asset('littlelink/images/avatar@2x.png 2x') }}" width="50px"
                                    height="50px">
                            </a>
                        @endif
                    @endforeach
