...
<head>
    ...
    @foreach ($information as $info)
        @php $GLOBALS['themeName'] = $info->theme; @endphp
    @endforeach
    ...
    @if (theme('enable_custom_code') == 'true' and
        theme('enable_custom_head') == 'true' and
        env('ALLOW_CUSTOM_CODE_IN_THEMES') == 'true')
        @include($GLOBALS['themeName'] . '.extra.custom-head')
    @endif
    ...
    @foreach ($information as $info)
        @if ($info->theme != '' and $info->theme != 'default')
            ...
            <!-- Theme details: -->
            <meta name="designer" href="{{ url('') . '/theme/@' . $littlelink_name }}"
                content="{{ url('') . '/theme/@' . $littlelink_name }}">
...
<body>
    ...
    @if (theme('enable_custom_code') == 'true' and
        theme('enable_custom_body') == 'true' and
        env('ALLOW_CUSTOM_CODE_IN_THEMES') == 'true')
        @include($GLOBALS['themeName'] . '.extra.custom-body')
    @endif
    ...
    @if ($ShowShrBtn == 'true')
        ...
        @if ($user_browser === 'Chrome' or get_operating_system() == 'mobile')
            ...
            <div align="right" class="sharediv">
                <div><span class="sharebutton button-hover icon-hover" id='share-share-button'><i
                            style="color: black;"
                            class="fa-solid fa-share sharebutton-img share-icon hvr-icon"></i><span
                            class="sharebutton-mb">Share</span></span></div>
            </div>
        ...
        @else
            <span class="copy-icon" role="button">
                <div onclick="alert('URL has been copied to your clipboard!')" align="right" class="sharediv">
                    <div><a class="sharebutton button-hover icon-hover"><i style="color: black;"
                                class="fa-solid fa-share sharebutton-img share-icon hvr-icon"></i><span
                                class="sharebutton-mb">Share</span></a></div>
                </div>
            </span>
        @endif
    ...
    <div class="container">
        <div class="row">
            <div class="column" style="margin-top: 5%">
                <!-- Your Image Here -->
                @if (file_exists(base_path("img/$littlelink_name" . '.png')))
                    <img alt="avatar" class="rounded-avatar fadein"
                        src="{{ asset("img/$littlelink_name" . '.png') }}" width="128px" height="128px"
                        style="object-fit: cover;">
                @elseif(file_exists(base_path('littlelink/images/avatar.png')))
                    <img alt="avatar" class="rounded-avatar fadein"
                        src="{{ asset('littlelink/images/avatar.png') }}"
                        srcset="{{ asset('littlelink/images/avatar@2x.png 2x') }}" width="128px" height="128px"
                        style="object-fit: cover;">
                @else
                    <img alt="avatar" class="rounded-avatar fadein"
                        src="{{ asset('littlelink/images/logo.svg') }}"
                        srcset="{{ asset('littlelink/images/avatar@2x.png 2x') }}" width="128px" height="128px"
                        style="object-fit: cover;">
                @endif
                ...
                <!-- Short Bio -->
                <center>
                    <p style="width: 50%; min-width: 300px;" class="fadein">
                        @if (env('ALLOW_USER_HTML') === true)
                            {!! $info->littlelink_description !!}@else{{ $info->littlelink_description }}
                        @endif
                    </p>
                </center>
                ...
                @foreach ($links as $link)
                    ...
                    @if ($link->button_id === 0)
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-title button button-hover icon-hover"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif>{{ $link->title }}</a></div>
                    @elseif($link->name === 'phone')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-default button button-hover icon-hover"
                                rel="noopener noreferrer nofollow" href="{{ $link->link }}"><img alt="button-icon"
                                    class="icon hvr-icon"
                                    src="@if (theme('use_custom_icons') == 'true') {{ url('themes/' . $GLOBALS['themeName'] . '/extra/custom-icons') }}/phone{{ theme('custom_icon_extension') }} @else{{ asset('\/littlelink/icons\/') }}phone.svg @endif"></i>{{ $link->title }}</a>
                        </div>
                    @elseif($link->name === 'custom' and $link->custom_css === '' or
                        $link->custom_css === 'NULL' or
                        theme('allow_custom_buttons') == 'false' and $link->name === 'custom')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-{{ $link->name }} button button-hover icon-hover"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif><i
                                    style="color: {{ $link->custom_icon }}"
                                    class="icon hvr-icon fa {{ $link->custom_icon }}"></i>{{ $link->title }}</a>
                        </div>
                    @elseif($link->name === 'custom' and $link->custom_css != '')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-hover icon-hover" style="{{ $link->custom_css }}"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif><i
                                    style="color: {{ $link->custom_icon }}"
                    ...
                    @elseif($link->name === 'buy me a coffee')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-coffee button button-hover icon-hover"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif><img alt="button-icon"
                                    class="icon hvr-icon"
                                    src="@if (theme('use_custom_icons') == 'true') {{ url('themes/' . $GLOBALS['themeName'] . '/extra/custom-icons') }}/coffee{{ theme('custom_icon_extension') }} @else{{ asset('\/littlelink/icons\/') }}coffee.svg @endif">Buy
                                me a Coffee</a></div>
                    @elseif($link->name === 'custom_website' and $link->custom_css === '' or
                        $link->custom_css === 'NULL' or
                        theme('allow_custom_buttons') == 'false' and $link->name === 'custom')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a
                                class="button button-custom_website button button-hover icon-hover"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif><img alt="button-icon"
                                    class="icon hvr-icon"
                                    src="https://icons.duckduckgo.com/ip3/{{ strp($link->link) }}.ico">{{ $link->title }}</a>
                        </div>
                    @elseif($link->name === 'custom_website' and $link->custom_css != '')
                        <div style="--delay: {{ $initial++ }}s" class="button-entrance">
                            <a class="button button-hover icon-hover" style="{{ $link->custom_css }}"
                                rel="noopener noreferrer nofollow"
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif>
                                <img alt="button-icon" class="icon hvr-icon"
                                    src="https://icons.duckduckgo.com/ip3/{{ strp($link->link) }}.ico">{{ $link->title }}</a>
                        </div>
                    ...
                    @elseif($link->name === 'heading')
                        <h2>{{ $link->title }}</h2>
                    @else
                        <?php include base_path('config/button-names.php');
                        $newLinkName = $linkName;
                        $isNewName = 'false';
                        foreach ($buttonNames as $key => $value) {
                            if ($newLinkName == $key) {
                                $newLinkName = $value;
                                $isNewName = 'true';
                            }
                        } ?>
                        <div style="--delay: {{ $initial++ }}s; display: flex;justify-content: center;"
                            class="button-entrance">
                            <a class="button button-{{ $link->name }} button-hover icon-hover" {{-- style="{{ $link->custom_css }}" rel="noopener noreferrer nofollow" --}}
                                href="{{ route('clickNumber') . '/' . $link->id . '/' . $link->link }}"
                                style="display: flex !important;justify-content: space-around;align-items: center;"
                                @if (theme('open_links_in_same_tab') != 'true') target="_blank" @endif>
                                <img alt="button-icon" class="hvr-icon" width="70"
                                    src="@if (theme('use_custom_icons') == 'true') {{ url('themes/' . $GLOBALS['themeName'] . '/extra/custom-icons') }}/{{ $linkName }}{{ theme('custom_icon_extension') }}
                                      @else{{ asset('\/littlelink/images/links\/') . $link->id }}.png @endif">
                                {{-- @if ($isNewName == 'true')
                                    {{ $newLinkName }}@else{{ ucfirst($newLinkName) }}
                                @endif --}}
                                {{ $link->title }}
                            </a>
                        </div>
    ...
    @if (theme('enable_custom_code') == 'true' and
        theme('enable_custom_body_end') == 'true' and
        env('ALLOW_CUSTOM_CODE_IN_THEMES') == 'true')
        @include($GLOBALS['themeName'] . '.extra.custom-body-end')
    @endif
