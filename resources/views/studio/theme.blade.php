...
@section('content')
    @foreach ($pages as $page)
        <h2 class="mb-4"><i class="bi bi-brush"> Selecciona un theme</i></h2>
        ...
        <form action="{{ route('editTheme') }}" enctype="multipart/form-data" method="post">
            ...
            <div id="result"
                style="left: 1%; position: relative; background-color:#2c2d3a; border-radius: 25px; min-width:300px; max-width:950px; box-shadow: 0 10px 20px -10px rgba(0,0,0, 0.6);">
                <div style="padding:5%5%;">
                    <h3 align="center" style="color:white">Previsualizar:</h3>
                    <center><img style="width:95%;max-width:700px;argin-left:1rem!important;"
                            src="@if (file_exists(base_path() . '/themes/' . $page->theme . '/preview.png')) {{ url('/themes/' . $page->theme . '/preview.png') }}@elseif($page->theme === 'default' or empty($page->theme)){{ url('/littlelink/images/themes/default.png') }}@else{{ url('/littlelink/images/themes/no-preview.png') }} @endif"></img>
                    </center>
                </div>
            </div><br>
            ...
            <button type="submit" class="mt-3 ml-3 btn btn-info">Actualizar theme</button>
        </form>
        </details>
        ...
        @if (auth()->user()->role == 'admin')
            @if (env('ENABLE_THEME_UPDATER') == 'true')
                <style>
                    details {
                        width: 65%;
                        margin-left: 15px;
                        {{-- max-width: calc(100% - 20rem); --}} position: relative;
                ...
                <details>
                    ...
                    <div class="content" style="padding:10px;">
                        <table>
                            <tr>
                                <th style="width:85%;">Nombre de theme:</th>
                                <th style="width: 15%;">Estado de atualización:</th>
                                <th>Version:&nbsp;</th>
                            </tr>
                            <?php
                    ...
                    <a href="{{ url('update/theme') }}" onclick="updateicon()" class="mt-3 ml-3 btn btn-info row"><span
                            id="updateicon" class=""><i class="bi bi-arrow-repeat"></i></span> Actualizar todos los
                        themes</a><br><br>
                    <script>
                        function updateicon() {
                ...
                <?php
                try {
                    if ($GLOBALS['updateAv'] == true) {
                        echo '<img style="padding-left:40px; padding-top:15px; scale: 1.5;" src="https://img.shields.io/static/v1?label=&message=A theme needs updating&color=brightgreen">';
                    }
                } catch (Exception $ex) {
                }
                ?>
            ...
            <form action="{{ route('editTheme') }}" enctype="multipart/form-data" method="post">
                ...
                <a class="deltheme" href="{{ url('/panel/theme') }}">&emsp; Eliminar themes</a>
                <div class="row">
                    <button type="submit" class="mt-3 ml-3 btn btn-info">Subir theme</button>
                    <button class="mt-3 ml-3 btn btn-primary" title="Download more themes">
                        <a href="https://littlelink-custom.com/themes.php" target="_blank" style="color:#FFFFFF;">Descargar
                            themes</a></button>
                </div>
            </form>
            </details>
